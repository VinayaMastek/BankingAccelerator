...
public class CustomerPersonaService {
	...
	@Path("{customerId}/preference/view/{viewId})")
	public CustomerDetails getCustomerPreference(
			@PathParam("customerId") String customerId,
			@PathParam("viewId") String viewId) {

		CustomerDetails customerDetails = new CustomerDetails();
		// for a customer id find the accounts
		JSONObject customerAcctDet = new JSONObject(
				customerservice.getOtherDetails(customerId));
		System.out.println(customerAcctDet.toString());
		JSONArray accounts = customerAcctDet.getJSONArray("accounts");

		for (Object obj : accounts) {

			JSONObject acct = (JSONObject) obj;
			System.out.println("Account" + acct.getString("id"));
			JSONObject acctTransactions = new JSONObject(
					customerservice.getAccountTransactions(customerId,acct.getString("id")));
			JSONArray transactions = acctTransactions
					.getJSONArray("allAccounts");
